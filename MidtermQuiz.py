...
# a function parameter is a parameter is the variable listed inside the parentheses in the function definition and a funtion argument is an argument is the value that are sent to the function when it is called.

# 2. In your own words, describe what each of the following errors means.
# Write your response using complete sentences.

# syntax error: something like a mispelled word or comma, smething along thoes lines
# type error: an operand or argument passed to a function is incompatible with the type expected by that operator or function
# name error: If the syntax omits double quotation marks for a text value
